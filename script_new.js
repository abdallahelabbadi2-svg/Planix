...
function startPomodoro() {
    ...
    // إضافة تأثير حركي للمؤقت
    document.querySelector('.pomodoro-timer').classList.add('active');
...
function pausePomodoro() {
    if (pomodoroTimer) {
        ...
        // إزالة تأثير حركي للمؤقت
        document.querySelector('.pomodoro-timer').classList.remove('active');
    }
}
...
function createProgressChart() {
    ...
    // تحديث الرسم البياني بالبيانات الحالية
    updateProgressChart();
}
...
function updateProgressChart() {
    ...
    progressChart.data.datasets[1].data = totalData;

    // تحديث التسميات إذا تغيرت اللغة
...
function loadTasksFromStorage() {
    ...
    if (savedLang) {
        ...
        document.getElementById(`lang-${savedLang === 'ar' ? 'en' : 'ar'}`).classList.remove('active');
    }
}
...
function toggleTheme() {
    ...
    if (body.classList.contains('dark-mode')) {
        ...
        localStorage.setItem('planix_theme', 'dark');
    } else {
        themeIcon.classList.remove('fa-sun');
        themeIcon.classList.add('fa-moon');
        localStorage.setItem('planix_theme', 'light');
    }

    // تحديث الرسم البياني ليتناسب مع الوضع الجديد
    updateChartTheme();
}

// تحميل الوضع الليلي المحفوظ
function loadTheme() {
    const savedTheme = localStorage.getItem('planix_theme');
    const themeIcon = document.querySelector('#theme-toggle i');
...
// تحديث موضوع الرسم البياني
function updateChartTheme() {
    if (progressChart) {
        const isDarkMode = document.body.classList.contains('dark-mode');

        progressChart.options.scales.x.ticks.color = isDarkMode ? '#f5f5f5' : '#333';
        progressChart.options.scales.y.ticks.color = isDarkMode ? '#f5f5f5' : '#333';
        progressChart.options.scales.x.grid.color = isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)';
        progressChart.options.scales.y.grid.color = isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)';
        progressChart.options.plugins.legend.labels.color = isDarkMode ? '#f5f5f5' : '#333';
...
function addInitialAnimations() {
    // إضافة تأثيرات متتالية للأقسام
    const sections = document.querySelectorAll('section');
    sections.forEach((section, index) => {
        section.style.animationDelay = `${index * 0.2}s`;
    });

    // إضافة تأثيرات متتالية لأعمدة الأيام
    const dayColumns = document.querySelectorAll('.day-column');
    dayColumns.forEach((column, index) => {
        column.style.animationDelay = `${index * 0.1}s`;
        column.style.animation = 'fadeIn 0.8s ease forwards';
